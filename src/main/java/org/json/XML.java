...
public class XML {
    ...
    public static JSONObject toJSONObject(Reader reader, JSONPointer path) {
        ...
        while (x.more()) {
            x.skipPast("<");
            if (x.more()) {
                parse(x, jo, null, XMLParserConfiguration.ORIGINAL, 0, path);
            }
        }
    ...
    private static boolean parse(XMLTokener x, JSONObject context, String name, XMLParserConfiguration config,
            int currentNestingDepth, JSONPointer path)
            throws JSONException {
        char c;
        int i;
        JSONObject jsonObject = null;
        ...
        String[] insidePath = parentPath.split("/");
        ...
        if (token == BANG || token == QUEST) {
            ...
            if (token == BANG || token == QUEST) {
                x.skipPast("-->");
                return false;
            } else if (token == insidePath[insidePath.length - 1]) {
                x.skipPast("</" + insidePath[0] + ">");
                return false;
            }
                        ...
                        if (token == null) {
                            ...
                            if (parse(x, jsonObject, tagName, config, currentNestingDepth + 1)) {
                                if (config.getForceList().contains(tagName)) {
                                    ...
                                    if (jsonObject.length() == 0) {
                                        context.accumulate(tagName, "");
                                    } else if (jsonObject.length() == 1 // here 2
                                            && jsonObject.opt(config.getcDataTagName()) != null) {
                                        context.accumulate(tagName, jsonObject.opt(config.getcDataTagName()));
                                    } else {
                                        context.accumulate(tagName, jsonObject);
