...
public class JSONObject {
    ...
    public Stream<JSONObject> toStream() {
        return StreamSupport.stream(Spliterators.spliteratorUnknownSize(new JSONDFSIterator(this),
                Spliterator.ORDERED), false);
    }

    public class JSONDFSIterator implements Iterator<JSONObject> {

        private final Stack<Object> stack;

        public JSONDFSIterator(JSONObject jsonObject) {
            this.stack = new Stack<>();
            stack.push(jsonObject); // Push the root JSON object onto the stack
        }
        ...
        public JSONObject next() {
            ...
            if (current instanceof JSONObject) {
                ...
                // Push the elements of the array onto the stack in reverse order for DFS
                for (int i = jsonArray.length() - 1; i >= 0; i--) {
                    stack.push(jsonArray.get(i));
                }
                return null; // Null indicates that an array is being processed, not a JSON object
            } else {
                // Invalid JSON type, skip
                return null;
            }
        }
    }
